    ...
    for evenement in pygame.event.get():
        ...
        if evenement.type == pygame.QUIT:
            ...
            if evenement.key == pygame.K_BACKSPACE:
                ...
                if(Option): # Gere les entrées dans le sous menu option
                    if select[1] == 12:
                        Option = False
...
def affiche_menu():
    ...
    if Option: # Affiche le menu d'option
        select[1] %= 13
        select[0] %= len(options)//12 + 1

        if select[1] == 12:
            dessiner_fleche((FENETRE_LARGEUR//2 - 170, 19*FENETRE_HAUTEUR//20) ,NOIR, 80, 16, 16, 2)
            dessiner_fleche((FENETRE_LARGEUR//2 - 170, 19*FENETRE_HAUTEUR//20) ,VERT, 80, 16, 16)

        else:
            dessiner_fleche((70 + select[0]*FENETRE_LARGEUR//3,  6*FENETRE_HAUTEUR//20 + select[1]*(1*FENETRE_HAUTEUR//20)),NOIR, 80, 16, 16, 2)
            dessiner_fleche((70 + select[0]*FENETRE_LARGEUR//3,   6*FENETRE_HAUTEUR//20 + select[1]*(1*FENETRE_HAUTEUR//20)),VERT, 80, 16, 16)
        ...
        for i in range(0, len(options)):
            if i > 11:
                j = i%11
                affiche_message(options_noms[i].format(options[i]), police, (20 + FENETRE_LARGEUR//6 + (i//12)*FENETRE_LARGEUR//3, (j+5)*FENETRE_HAUTEUR//20), NOIR)
            else:
                affiche_message(options_noms[i].format(options[i]), police, (20 + FENETRE_LARGEUR//6, (i+6)*FENETRE_HAUTEUR//20), NOIR)
        ...
        dessiner_fleche((7*FENETRE_LARGEUR//20, 11*FENETRE_HAUTEUR//20 + select[1]*(3*FENETRE_HAUTEUR//20)),NOIR, 80, 16, 16, 2)
        dessiner_fleche((7*FENETRE_LARGEUR//20, 11*FENETRE_HAUTEUR//20 + select[1]*(3*FENETRE_HAUTEUR//20)),VERT, 80, 16, 16)
...
def maj_positions():
    ...
    k = 0
    for avion in avions:
        if avion['vie'] <= 0: # Animation de crash
            global t, ori, first
            if first[i]:
                crash.play()
        ...
        i += 1
        # Affiche l'image d'explosion et le son correspondant si l'avion touche le sol
...
def maj_balles(): # Mets à jour la position des balles
    i = 0
    for avion in avions:
...
def affiche_nuage(): # Les nuages qui sortent de la fenêtre reviennent de l'autre coté
    i = 0
    for nuage in nuages:
...
def affiche_vies(): # Affiche la barre de vie pendant un cours delai si l'avion est touché
    for avion in avions:
        if(avion['vie'] < 0):
            avion['vie'] = 0
...
def dessiner_sol1(temps): # Sol à l'avant
...
def dessiner_sol2(temps): # Sol à l'arrière plan
    for x in range(dimensions_fenetre[0]):
...
def collision():
    ...
    for avion1 in avions:
        j = 0
        for power in power_up_list: # Collision entre un avion et un power_up
        ...
        for avion2 in avions:
            if COLLISION: # Collision entre les avions si l'option est activée
            ...
            i = 0
            for balle in avion1['balles']: # Collision entre les balles et les avions
...
def fait_nuage(): # Créé 14 nuages aléatoirement sur la fenêtre
    global nuages
    i = 0
    while (i < 14):
...
def reinitialiser(new=False): # Reinitalise l'aire de jeu (reinitalise aussi les scores si l'argument est True)
    ...
    if not Jeu:
        ...
        if not Pause:
            ...
            if POWER:
                power_up()
        fenetre.fill(BLEU_CIEL)
        dessiner_sol2(temps_maintenant/20) # Se déplace plus lentement que le sol1
        affiche_nuage()
        affiche_vies()
        affiche_balles()
        affiche_avions()
